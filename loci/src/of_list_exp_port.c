...
int
of_list_exp_port_first(of_list_exp_port_t *list, of_object_t *obj)
{
    int rv;

    of_exp_port_init(obj, list->version, -1, 1);
...
int
of_list_exp_port_next(of_list_exp_port_t *list, of_object_t *obj)
{
    int rv;
...
/**
 * Set up to append an object of type of_exp_port to an of_list_exp_port.
...
int
of_list_exp_port_append_bind(of_list_exp_port_t *list, of_object_t *obj)
{
    return of_list_append_bind(list, obj);
}

/**
 * Append an object to a of_list_exp_port list.
 *
...
int
of_list_exp_port_append(of_list_exp_port_t *list, of_object_t *obj)
{
    return of_list_append(list, obj);
}

/**
 * \defgroup of_list_exp_port of_list_exp_port
 */

/**
 * Create a new of_list_exp_port object
 *
 ...
 *
 * \ingroup of_list_exp_port
 */

of_object_t *
of_list_exp_port_new(of_version_t version)
{
    of_object_t *obj;
    int bytes;

    bytes = of_object_fixed_len[version][OF_LIST_EXP_PORT];

    if ((obj = of_object_new(OF_WIRE_BUFFER_MAX_LENGTH)) == NULL) {
        return NULL;
    }

    of_list_exp_port_init(obj, version, bytes, 0);

    return obj;
}

/**
 * Initialize an object of type of_list_exp_port.
 *
...
void
of_list_exp_port_init(of_object_t *obj,
    of_version_t version, int bytes, int clean_wire)
{
    LOCI_ASSERT(of_object_fixed_len[version][OF_LIST_EXP_PORT] >= 0);
    if (clean_wire) {
    ...
    obj->version = version;
    obj->length = bytes;
    obj->object_id = OF_LIST_EXP_PORT;
